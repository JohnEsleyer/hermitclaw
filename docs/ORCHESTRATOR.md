...
- `createNewCubicle()`: Handles the creation of the **Standardized Portal** folders (`/out`, `/in`, `/www`) on the host filesystem for each agent/user pair.
...
- **Tables**: Agents, Budgets, Allowlist, Settings, Audit Logs, Agent Memory, Runtime Logs.
